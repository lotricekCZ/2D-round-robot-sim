...
all: $(EXE) $(OBJ_DIR)/main.o $(BIN_DIR)/libpoint.so $(BIN_DIR)/libvector.so $(BIN_DIR)

run: all
	@LD_LIBRARY_PATH=$(BIN_DIR) ./$(EXE)

$(EXE): $(OBJ_DIR)/main.o $(BIN_DIR)/libpoint.so $(BIN_DIR)/libvector.so $(BIN_DIR)/libline.so | $(BIN_DIR)
	$(CC) -L $(BIN_DIR) -o $@ $^ $(LDLIBS) -I$(BIN_DIR) -lpoint -lvector -lline
